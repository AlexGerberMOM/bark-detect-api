...
# Жесткие ограничения памяти для стабильной работы в Яндекс Облаке
...
# Инициализируем обе сессии ONNX
...
async def predict_wood_biomass(file: UploadFile = File(...)):
    ...
    try:
        # Пишем входящий поток в файл на диск
        ...
        # Оптимизируем размер кадра под таксационный стандарт (512х512)
        clean_img = cv2.resize(clean_img, (512, 512), interpolation=cv2.INTER_AREA)
        h_orig, w_orig, _ = clean_img.shape
        
        # =========================================================================
        # СТУПЕНЬ 1: ЛОКАЛИЗАЦИЯ ТОРЦА И ВЫЧИСЛЕНИЕ ЦЕНТРА СЕРДЦЕВИНЫ
        # =========================================================================
        ...
        # =========================================================================
        # ГЕОМЕТРИЧЕСКИЙ ТРАНСФОРМЕР: ПОЛЯРНАЯ ЛИНЕАРИЗАЦИЯ ОКРУЖНОСТИ
        # =========================================================================
        ...
        # =========================================================================
        # СТУПЕНЬ 2: СЕГМЕНТАЦИЯ СЛОЯ КОРЫ НА ПОЛЯРНОЙ ЛЕНТЕ (ВЕРНУТА)
        # =========================================================================
        ...
        # =========================================================================
        # ОБРАТНЫЙ РЕМАППИНГ И ФИНАЛЬНЫЙ ТАКСАЦИОННЫЙ ЛУЧЕВОЙ КАСТИНГ 360°
        # =========================================================================
        ...
        # Лучевое зондирование толщины коры
        ray_thicknesses = []
        ...
        # Наложение полупрозрачной красной маски коры на исходный кадр
        ...
        # Генерация аварийной картинки для защиты смартфона от вылета
